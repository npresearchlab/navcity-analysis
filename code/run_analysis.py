...
def run_metrics_calculation(data_folder: str, participant_ids: List[str]) -> None:
    """
    ...
    for pid in participant_ids:
        for block_num in range(1, 4):
            ...
            try:
                ...
                output_path = f"{data_folder}/{pid}/b{block_num}_results.csv"
...
def run_merge(data_folder: str, participant_ids: List[str]) -> None:
    """
    ...
    merged_df = merge_block_results(data_folder, participant_ids)

    if not merged_df.empty:
        output_path = f"{data_folder}/merged_results.csv"
...
def run_average(data_folder: str, participant_ids: List[str]) -> None:
    """
    ...
    averaged_df = average_metrics(data_folder, participant_ids)

    if not averaged_df.empty:
        output_path = f"{data_folder}/averaged_results.csv"
...
def run_trajectories(data_folder: str, participant_ids: List[str]) -> None:
    """
    ...
    extract_target_trajectories(data_folder, participant_ids)


def run_plots(data_folder: str, participant_ids: List[str]) -> None:
    """
    ...
    generate_participant_movement_plots(data_folder, participant_ids)

    print("\nGenerating target maps...")
    plot_target_maps(data_folder)


def run_post_process(base_data_folder: str) -> None:
    """
    ...
    post_analysis_cleanup(base_data_folder)
...
def process_data_folder(
    ...
    # Run requested steps
    if 'metrics' in steps:
        run_metrics_calculation(data_folder, participant_ids)

    if 'merge' in steps:
        run_merge(data_folder, participant_ids)

    if 'average' in steps:
        run_average(data_folder, participant_ids)

    if 'trajectories' in steps:
        run_trajectories(data_folder, participant_ids)

    if 'plots' in steps:
        run_plots(data_folder, participant_ids)
    ...
    if args.data_folders:
        ...
        for folder in args.data_folders:
            if not os.path.exists(folder):
                ...
                continue
            process_data_folder(folder, non_post_steps)
